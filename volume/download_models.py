'''
Since containers with published ports don't have internet, create a file to download stuff
'''
import torch
import re
...
detr_hubconf_url = 'https://raw.githubusercontent.com/facebookresearch/detr/master/hubconf.py'
detr_hubconf = requests.get(detr_hubconf_url).text

models: Sequence[str] = re.findall('def\s(detr_\w+)\(', detr_hubconf)

print(f'Found {len(models)} models in hubconf:')
pprint(models)
...
for model in models:
    ...
    print(f'{f"":=^{print_width}}')
    torch.hub.load('facebookresearch/detr', model, pretrained=True)
