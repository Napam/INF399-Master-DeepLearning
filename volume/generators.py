...
class BlenderStandardDataset(BlenderDatasetBase):
    ...
    def get_batch_images(self, batch_indices: Iterable):
        return [self.get_image(os.path.join(self.img_dir, f"img{i}.png")) for i in batch_indices]

    def get_batch(
        self, batchnr: int
    ) -> Tuple[
        List[Tuple[Sequence[Sequence[Sequence[float]]]]],
        List[Tuple[Sequence[int], Sequence[Sequence[float]]]],
    ]:
        """
        Get batch
...
class Blender3DDataset(BlenderStereoDataset):
    """
...
class TorchStereoDataset(BlenderStereoDataset):
    ...
    def get_batch(
        ...
        List[Sequence[Sequence[Sequence[float]]]],
        List[Tuple[Sequence[int], Sequence[Sequence[float]]]],
    ]:
        X_batch, y_batch = super().get_batch(batchnr)
        ...
        X_batch: Tuple[torch.Tensor] = (
            np_to_torch_img(X_batch[0]).unsqueeze(0).to(self.device),
...
class Torch3DDataset(Blender3DDataset):
    def __init__(
        self,
        data_dir: str,
        table: str,
